...
function Papers_status_last_upload_date() {
  ...
  useEffect(() => {
    ...
    if (conference_id) {
      report_fetchpaperstatus(conference_id).then((res) => {
        setData(res.data);
      }).catch((err) => {
        ...
        <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
          <thead className="ltr:text-left rtl:text-right">
            <tr>
              ...
              <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Keywords</th>
              <th className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">Review By</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {data.map((item, index) => (
              <tr >
                ...
                <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{item.keywords}</td>

                <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{item.status}</td>

                <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{item.ldou}</td>
